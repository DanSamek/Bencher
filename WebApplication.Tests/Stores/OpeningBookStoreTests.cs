...
public class OpeningBookStoreTests : TestBase
{
    ...
    public void GetOpeningBooksForUser()
    {
        ...
        Assert.That(books[0].Name, Is.Not.EqualTo("test-book1").Or.Not.EqualTo("test-book-3"));
        Assert.That(books[1].Name, Is.Not.EqualTo("test-book1").Or.Not.EqualTo("test-book-3"));
    }
    ...
    public void Add()
    {
        ...
        var openingBooks = Factory.CreateDbContext().OpeningBooks.ToArray();
        Assert.That(openingBooks, Has.Length.EqualTo(1));

        var book = openingBooks[0];
        Assert.That(book.Data, Is.EqualTo(new byte [] {0x1, 0x2, 0x4}));
